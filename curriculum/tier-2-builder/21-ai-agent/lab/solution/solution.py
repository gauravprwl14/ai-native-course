        ...
        self.tool_definitions = [
            ...
            },
            {
                ...
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "expression": {"type": "string", "description": "Python math expression, e.g. 'math.sqrt(144)'"}
                    },
                    "required": ["expression"]
                }
            },
            {
                "name": "read_file",
                "description": "Read the contents of a local file (mock).",
                "input_schema": {
                    "type": "object",
                    "properties": {
    ...
    def run(self, task: str) -> AgentResult:
        ...
        for i in range(self.max_iterations):
            ...
            if response.stop_reason == "tool_use":
                ...
                print(f"[Step {steps}] {tool_block.name}({tool_block.input}) -> {result[:80]}")

                is_error = (
                    result.startswith("Error:")
                    or result.startswith("Unknown tool:")
                    or result.startswith("Tool error:")
                    or result.startswith("Calculation error:")
                )
                if is_error:
