...
def stream_response(prompt: str, system_prompt: str = None) -> Generator[str, None, None]:
    ...
    Yields each text chunk as it arrives from the Anthropic API.
...
def stream_and_collect(prompt: str) -> dict:
    ...
    #     chunks.append(text)
    # return {"full_text": "".join(chunks), "chunk_count": len(chunks)}
    """
    raise NotImplementedError("Implement stream_and_collect")


def measure_ttft(prompt: str) -> float:
    """
    Measure time-to-first-token in milliseconds.
    Returns a float: milliseconds from API call start until first chunk received.
    Returns -1.0 if no response is received.
    ...
    """
    raise NotImplementedError("Implement measure_ttft")
